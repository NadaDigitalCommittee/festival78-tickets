...
export const RaffleNews: FC<Props> = ({ raffle, event }) => {
  ...
  if (raffle.result === Result.WIN) {
    return (
      <>
        <p>この度は、抽選券システムをご利用いただき誠に有難うございます。見事当選されましたことをご報告致します。</p>
        <h2>企画概要</h2>
        <p>
          企画名: {eventName}
        ...
        <p>企画に参加される場合は当選番号が必要です。入場前にスタッフから確認がありますので、上記のページをスマホで開いた上でご提示ください。</p>
      </>
    );
  } else {
    return (
      <>
        <p>この度は、抽選券システムをご利用いただき有難うございます。</p>
        <p>厳正なる抽選の結果、誠に残念ながら今回はご希望に添えず、落選となりましたことをご報告いたします。</p>
        <h2>企画概要</h2>
        <p>
          企画名: {eventName}
        </p>
